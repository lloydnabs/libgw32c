...
do_test (int argc, char *argv[])
{
  ...
  fsync(fd);
  if ((fl=fstat (fd, &st)) < 0 || st.st_size != sizeof (buf)) 
    error (EXIT_FAILURE, 0, "initial size wrong: %d", fl);
 
  if (FTRUNCATE (fd, 800) < 0)
    error (EXIT_FAILURE, errno, "size reduction with %s failed",
	   STRINGIFY (FTRUNCATE));

  if ((fl=fstat (fd, &st)) < 0 || st.st_size != 800)
    error (EXIT_FAILURE, 0, "size after reduction with %s incorrect: %I64d",
	   STRINGIFY (FTRUNCATE), fl);
  ...
  if (fl=fstat (fd, &st) < 0 || st.st_size != 800)
    error (EXIT_FAILURE, 0, "size after reduction with %s incorrect: %d",
	   STRINGIFY (TRUNCATE), fl);
  ...
  if (TRUNCATE (name, 5000000000) < 0)
    error (EXIT_FAILURE, errno, "size increase with %s failed",
	   STRINGIFY (TRUNCATE));

  if (fstat (fd, &st) < 0 || st.st_size != 5000000000)
    error (EXIT_FAILURE, 0, "size after increase with %s incorrect",
	   STRINGIFY (TRUNCATE));

  if (FTRUNCATE (fd, 5000000000) < 0)
    error (EXIT_FAILURE, errno, "size increase with %s failed",
	   STRINGIFY (FTRUNCATE));
  ...
  close (fd);
//  if (unlink (name) < 0)
//  	error (EXIT_FAILURE, errno, "cannot unlink %s", name);
